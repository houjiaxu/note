    ...
    对jar包进行解压,能看到里面有个文件夹MATA-INF,打开这个文件夹,里面有个MAINFEST.MF文件,里面有2个重要信息
        ...
        Start-Class: cn.com.duiba.creditsclub.Application
            ...
            prepareContext()
                ...
                applyInitializers(context);//循环调用所有的ApplicationContextInitializer.initialize方法
                ...
                load(context, sources.toArray(new Object[0]));//非必须使用AOT优化的时候才会调用这个方法,里面是创建了一些bean定义, 具体这个AOT是个什么东西我也不知道
