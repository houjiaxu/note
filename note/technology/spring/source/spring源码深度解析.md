...
假如没有二级缓存:
    ...
    假设存在A,B循环依赖的话, 给A注入B的时候,创建B,此时发现B依赖A,那么就要给B注入A,没有二级缓存,只能从三级缓存ObjectFactory#getObject重新创建一个A,那么就和原来的A不是同一个了,保证不了单例了

假如没有三级缓存,那么二级缓存应该存放什么样的对象?代理后的?还是代理前的?

    1.代理前的,存在循环依赖时就会有问题,给别人注入了一个没有代理后的对象;
    2.代理后的,那么代理后的对象从哪儿来? 总不能在此方法中循环调用bean后置处理器吧(三级缓存就是封装了循环调用bean的后置处理器), 这样可以解决问题, 
        但是代码太丑了,不符合spring的代码风格,所以spring将这块代码进行封装成一个lambda表达式,即ObjectFactory#getObject方法,在循环依赖的实现就是循环调用bean的后置处理器;

	spring三级缓存长这个样子Map<String, ObjectFactory<?>> singletonFactories
    spring二级缓存中存放的是三级缓存创建的对象,二级缓存中没有对象时,会取三级缓存中的lambda表达式进行创建,然后将创建好的对象放入二级缓存,三后删除三级缓存;
